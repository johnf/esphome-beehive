            ...
            for f in glob.glob(os.path.join(d, '*.fzp')):
                ...
                for m in list(missing):
                    if f'moduleId="{m}"'.encode() in data:
                        self._load_fzp(data)
                        missing.discard(m)
        # generic headers are generated by Fritzing at load time; name their pins
        for m in list(missing):
            if m.startswith('generic_') and 'pin_header' in m:
                n = int(m.split('_')[-2])
                self.pins[m] = {f'connector{k}': f'pin{k + 1}' for k in range(n)}
                self.buses[m] = []
                missing.discard(m)
